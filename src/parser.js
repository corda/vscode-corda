    ...
    for (; state.index < chunkLength; state.index++) {
        ...
        if (character === CHAR_BLOCK_START) {
            ...
            if (SPECIAL_KEYS.hasOwnProperty(currentKey)) {
                ...
                if (currentKey == 'node') {
                    //TODO this is such a quick and dirty hack to get multiple node entries parsed individually instead of being squashed into one struct
                    out[currentKey].push(deepParse(chunk, state, keepFunctionCalls, skipEmptyValues));
                } else {
