...
var openTerminals = []; // terminals holding run-node instances
var nodeLoaded = false;
var gradleTerminal = null;
var clientTerminal = null;
var webViewPanels = [];
var projectCwd = '';
/**
 ...
 */
function launchView(context, view) {
    // LAUNCH BACKEND
    launchViewBackend();
 ...
 */
function scanGradleFile(fileName, last) {
    gjs.parseFile(fileName).then(function (representation) {
        ...
        if (representation.task !== undefined && representation.task.node !== undefined) {
            if (representation.task.nodeDefaults) {
                nodeDefaults = representation.task.nodeDefaults;
            }
            else {
